...
class BaseNuisanceModule(pl.LightningModule):
    def __init__(
        self,
        model_config: FNO1dConfig | FNO2dConfig | DeepONet1dConfig | None = None,
...
class SolutionOperatorModule(BaseNuisanceModule):
    def _loss(self, batch: dict[str, Tensor]) -> Tensor:
        coeff = batch["coeff"]
        x_grid = batch["x_grid"]
        obs_x = batch["obs_x"]
        obs_y = batch["obs_y"]
        preds = self.predict_points(coeff, obs_x, x_grid)
        return torch.mean((preds - obs_y) ** 2)
...
class DebiasingOperatorModule(BaseNuisanceModule):
    def __init__(
        ...
        frozen_solution_model: NeuralOperator1D | DeepONet1D | NeuralOperator2D | None = None,
        mode: str = "full",
    ) -> None:
    ...
    def _loss(self, batch: dict[str, Tensor]) -> tuple[Tensor, Tensor]:
        coeff = batch["coeff"]
        x_grid = batch["x_grid"]
        obs_x = batch["obs_x"]
        ...
        if self.functional_name is not None and self.frozen_solution_model is not None and "u_hat" in batch:
            ...
            penalty = self._parameter_penalty()
    ...
    def _parameter_penalty(self) -> Tensor:
        """Return the paper's Riesz regularizer lambda * ||eta||_2^2."""
        parameters = list(self.model.parameters())
        if not parameters:
            return torch.zeros((), device=self.device)
        squared_norm = torch.stack([torch.sum(parameter ** 2) for parameter in parameters]).sum()
        return self.module_config.riesz_penalty_lambda * squared_norm

    def training_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        loss, penalty = self._loss(batch)
        self.log("train_loss", loss, prog_bar=False, on_step=False, on_epoch=True)
        self.log("riesz_penalty", penalty, prog_bar=False, on_step=False, on_epoch=True)
        return loss

    def validation_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        # functional_jvp uses autograd even though no optimizer step is taken.
        with torch.enable_grad():
            loss, penalty = self._loss(batch)
        self.log("val_loss", loss, prog_bar=False, on_step=False, on_epoch=True)
        self.log("val_riesz_penalty", penalty, prog_bar=False, on_step=False, on_epoch=True)
        return loss
