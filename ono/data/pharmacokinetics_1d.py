...
class Pharmacokinetics1DConfig:
    ...
    design_strength_scale: float
    design_strength_exponent: float
    peak_bandwidth: float
    design_center_time: float
    splits: dict[str, int]
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "Pharmacokinetics1DConfig":
        ...
        return cls(
            ...
            design_strength_scale=float(observations.get("design_strength_scale", 5.0)),
            design_strength_exponent=float(observations.get("design_strength_exponent", 1.0)),
            peak_bandwidth=float(observations.get("peak_bandwidth", 1.5)),
            design_center_time=float(observations.get("design_center_time", 18.0)),
...
def effective_design_strength(config: Pharmacokinetics1DConfig) -> float:
    scale = max(config.design_strength_scale, 1e-8)
    strength = max(config.design_strength, 0.0) ** max(config.design_strength_exponent, 1e-8)
    return float(np.clip(strength / scale, 0.0, 0.999))
...
def sample_observations(
    ...
    if config.observation_design != "fixed_window_mixture":
        raise ValueError("the paper PK generator supports only fixed_window_mixture")
    window_density = _window_peak_density(t_grid, config.design_center_time, config.peak_bandwidth)
    uniform = np.full_like(t_grid, fill_value=1.0 / t_grid.size, dtype=np.float64)
    gamma = effective_design_strength(config)
    design_prob = (1.0 - gamma) * uniform + gamma * window_density
...
def dataset_metadata(config: Pharmacokinetics1DConfig) -> dict[str, Any]:
    return {
        ...
        "observations": {
            ...
            "design_strength_scale": config.design_strength_scale,
            "design_strength_exponent": config.design_strength_exponent,
            "peak_bandwidth": config.peak_bandwidth,
            "design_center_time": config.design_center_time,
        },
